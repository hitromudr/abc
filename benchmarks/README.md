...
**Opus (сильная модель):** оба справились. GSD+Opus: 19/19 тестов, 277 LOC diff, 987s, 42 tool calls. Cadabra+Opus: 19/19, 266 LOC, 307s, 30 tool calls. Cadabra на 3× быстрее, но GSD не провалился.

**Flash (дешёвая модель):** GSD проваливается.
...
**Вывод:** cadabra не побеждает сильную модель — он позволяет заменить её на дешёвую. Cost router: `O(1 × expensive + n × cheap)` vs `O(n × expensive)`.

### Кросс-задачные выводы

| Класс задачи | Cynefin | Сильная модель | Дешёвая модель |
|-------------|---------|----------------|----------------|
| Code Audit | Complex | GSD ≈ abra | abra помогает |
| Bug Fix | Clear | GSD ✅ | GSD ✅ |
| Refactor | Complicated | GSD ✅ / Cadabra ✅ (быстрее) | GSD ❌ / **Cadabra ✅** |
| *Greenfield* | *Complex* | *не тестировалось* | *гипотеза: abra* |

**Главный вывод:** cadabra не побеждает сильную модель по качеству — он позволяет заменить её на дешёвую без потери результата. Cost router: `O(1 × expensive + n × cheap)` vs `O(n × expensive)`.
