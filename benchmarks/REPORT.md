...
Тест главной гипотезы: может ли DeepSeek ($0.017) с пошаговым оркестратором приблизиться к GSD+Opus ($0.70)?

### Что такое Cadabra Runtime

Python-скрипт (~200 LOC), который:
1. Берёт EXECUTION_STATE (DAG из 6 шагов)
2. Для каждого шага: вызывает DeepSeek API → применяет результат → запускает `py_compile`
3. При ошибке: retry с контекстом ошибки (max 3)
4. Финал: integration tests

### Результаты (3 прогона)

| Прогон | Steps | tests | compiles | API | diff | tokens | cost | time |
|--------|:-----:|:-----:|:--------:|:---:|-----:|-------:|-----:|-----:|
| ds-runtime | 5/6 | ❌ 17/19 | ✅ | ✅ | 511 | 42K | $0.014 | 479s |
| ds-runtime2 | 5/6 | ❌ 17/19 | ✅ | ✅ | 354 | 48K | $0.014 | 648s |
| ds-runtime3 | 5/6 | ❌ 17/19 | ✅ | ✅ | 332 | 61K | $0.017 | 472s |

**Стабильный паттерн:** 5 из 6 шагов DAG проходят, py_compile ✅ на каждом шаге, API preserved ✅. Но 2 из 19 integration тестов падают.

### Какие тесты падают

`test_single_gitignore_implementation` и `test_single_file_type_definition` — оба проверяют что старый код **удалён** из файлов-источников. DeepSeek создаёт правильный `file_utils.py` и обновляет imports, но **не зачищает** оригинальные определения (оставляет `pathspec` imports и extension sets в services.py/graph_builder.py). Это типичная проблема дешёвых моделей: создают новое, но не чистят старое.

### Сравнительная матрица bench 005

| Подход | Модель | tests | compiles | API | diff | cost | time |
|--------|--------|:-----:|:--------:|:---:|-----:|-----:|-----:|
| GSD interactive | Opus | ✅ 19/19 | ✅ | ✅ | 277 | ~$0.70 | 987s |
| Cadabra interactive | Opus | ✅ 19/19 | ✅ | ✅ | 266 | ~$0.75 | 307s |
| **Cadabra runtime** | **DeepSeek** | **❌ 17/19** | **✅** | **✅** | **~400** | **$0.015** | **~530s** |
| API one-shot | DeepSeek | ❌ 0/19 | ❌ | ❌ | 117 | $0.009 | 175s |

### Выводы

1. **API one-shot → runtime: 0/19 → 17/19.** Пошаговый оркестратор кардинально улучшает результат дешёвой модели
2. **$0.015 vs $0.70 — 47× дешевле Opus** при 89% success rate (17/19)
3. **Последние 2 теста** — проблема инструкции, не модели. Нужно в EXECUTION_STATE явно указать: "удали определения extensions из исходных файлов" (Kill Box наоборот — MUST_DO)
4. **compiles ✅ + API ✅** — модель не сломала ничего критического. Код рабочий, просто не полностью вычищен
