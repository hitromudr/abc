...
GSD-агент (Get Shit Done — стандартный паттерн Claude Code / Cursor) — монолит: одна модель одновременно архитектор, кодер и дебаггер. На Complicated-задачах (рефакторинг, миграции) дешёвые модели в GSD-режиме теряют контекст, ломают API и сжигают бюджет в зомби-циклах.

abracadabra разделяет роли: **abra** (архитектор) создаёт план с ограничениями (DAG + Kill Box + retry budget), **cadabra** (исполнитель) методично итерирует по плану. Результат: дешёвая модель решает задачу с 100% success rate vs 0% у GSD, в 3× дешевле.
...
API-режим показал 0% success. Интерактивный режим — cadabra vs GSD-агент (монолитный агент с полным доступом к инструментам).
...
**Почему GSD проваливается на Complicated-задачах:**
1. **Context Bloat** — после 3 ошибок окно забито стэктрейсами, модель теряет исходную задачу
2. **Мутация API** — GSD оптимизирует «зелёный свет в консоли», ломая обратную совместимость
3. **Зомби-циклы** — бесконечный fix→break→fix без ограничения, $1.50 и 15 попыток до max_turns
4. **`O(n × expensive)`** — каждая итерация по цене архитектора

**Почему cadabra работает:** EXECUTION_STATE.md = внешний файл с DAG + Kill Box + retry budget. Архитектура не в голове агента → не вымывается. Один вызов дорогой модели (abra, $0.02) + N итераций дешёвой (cadabra, $0.08) = `O(1 × expensive + n × cheap)`.
...
Bug fix — задача конкретная, критерий бинарный. Фреймворк = overhead. GSD (vanilla prompt) решает за $0.04–0.06.

**4. Cadabra побеждает GSD на Complicated-задачах: 100% vs 0%.**
GSD-монолит теряет контекст (Context Bloat), ломает API (нет Kill Box), сжигает бюджет в зомби-циклах ($0.30, 0% success). Cadabra с EXECUTION_STATE.md: $0.10, 100% success. В 3× дешевле.

**5. abracadabra = cost router для агентных задач.**
Не "улучшаем мышление LLM", а разделение ролей по стоимости:
- GSD: `O(n × expensive)` — одна модель думает + пишет + дебажит каждый turn
- Cadabra: `O(1 × expensive + n × cheap)` — дорогая модель один раз (abra), дешёвая итерирует (cadabra)

**6. Cynefin-маппинг (эмпирически подтверждён).**

| Класс | Cynefin | Оптимальный подход | Почему |
|-------|---------|-------------------|--------|
| Code Audit | Complex | abra ≈ GSD | Abra помогает слабым моделям |
| Bug Fix | Clear | GSD (vanilla) | Задача конкретная, фреймворк мешает |
...
**cadabra (тот же Gemini Flash + EXECUTION_STATE.md):**
```
Step 1/4: extract → file_utils.py                         ← DAG
Step 2/4: update imports → py_compile OK                   ← локальная верификация
Step 3/4: pytest → 1 fail → retry 1/3 → fix → green       ← retry budget
Step 4/4: cleanup → all green                              ← Kill Box: API не тронут
