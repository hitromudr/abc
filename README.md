...
**3. Abra вредит на code generation задачах (Clear-домен).**
Bug fix — задача конкретная, критерий успеха бинарный. Фреймворк добавляет overhead без прироста качества.

**4. Рефакторинг требует интерактивного агента (Complicated-домен).**
One-shot API не работает для refactor — модели создают правильную архитектуру, но ломают import paths. Нужен retry loop: apply patch → run tests → fix → retry. Это именно то, для чего спроектирован cadabra, но в API-режиме он лишён своего главного механизма.

**5. Ценность abra/cadabra — в маршрутизации, не в промпте.**
В zero-shot API abra ≈ baseline. Сила раскрывается в **интерактивных агентных средах** (Claude Code, Cursor, Zed):
- Approval Gate останавливает перед необратимыми действиями
- EXECUTION_STATE.md изолирует зону поражения
- cadabra выполняет фикс атомарно с retry loop и тестами

**6. Класс задачи определяет эффект.**

| Класс | Cynefin | Результат | Эффект abra |
|-------|---------|-----------|-------------|
| Code Audit | Complex | abra 6 / baseline 7 | ≈ паритет, помогает слабым моделям |
| Bug Fix | Clear | abra 0 / baseline 2 | вреден — overhead без прироста |
| Refactor | Complicated | все 0 (one-shot) | требует интерактивного агента |
